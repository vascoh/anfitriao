...
- [ ] **`RESEND_API_KEY`** — sem ela **não sai um único email** em produção (`NoopProvider` engole tudo; confirmado nos logs de arranque a 12/08). Afeta pedidos e confirmações de reserva, check-in, lembretes de pagamento, fim de trial, alertas de conformidade, relatório mensal e o motor de automações. ✅ **`EMAIL_FROM` já está definida** em produção desde 12/08 (confirmado por `vercel env ls production` a 07/09) — resta o remetente estar verificado no Resend, senão a chave entra e os emails ficam a ser recusados na mesma
