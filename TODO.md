...
- [x] Sitemap + Schema.org por tenant — **decisão do utilizador (2026-07-27): sites indexáveis**. Todas as páginas `/r/[slug]` (home, sobre, galeria, localização, blog+posts, legal) trocaram `noindex`→`index` + `alternates.canonical`; `/r/[slug]/sitemap.xml` novo (home + páginas + reservas ativas + posts); `robots.ts` raiz lista um sitemap por tenant ativo; Schema.org `LodgingBusiness` (JSON-LD) na homepage, um por alojamento. **Bug crítico encontrado e corrigido no mesmo incremento**: `/robots.txt` e `/sitemap.xml` raiz estavam bloqueados pelo middleware do Clerk desde a sua criação original (nunca estiveram na lista de rotas públicas) — o Google nunca conseguiu ler nenhum dos dois em produção, isto não tinha relação com o `noindex` das páginas. Corrigido em `src/proxy.ts`
...
8. ~~Os sites `/r/[slug]` devem passar a ser indexáveis pelo Google?~~ — **decidido (2026-07-27): sim**, ver Fase 2 acima
