...
- [x] **1.4 Taxa turística: 5 → 12 concelhos** — **fechado a 2026-09-25**: Vila Nova de Gaia (Reg. 195/2024 + 3.ª alteração, 3 € desde 21-08-2026), Braga (Reg. 927/2025), Portimão (Aviso 5384/2024/2), Mafra (Reg. 859-A/2018 alt. 207/2023, valores 2026 do site do município) e Óbidos (DR 2.ª série n.º 219/2018), todos por Diário da República ou site oficial do município. O modelo passou a ter **datas de vigência** (`alteracoes`), para uma subida a meio do ano não recalcular as estadias anteriores ao valor novo, `valoresDesde` (aviso para estadias anteriores ao valor verificado — Mafra atualiza todos os anos) e `aliases` («Gaia»). **Deixados de fora de propósito**: Coimbra (subida para 2 € aprovada a 21-07-2026 sem data de entrada em vigor), Lagoa (existe no Algarve e nos Açores, com taxas diferentes — o nome não chega para escolher), Lagos (sem TMT), Funchal (regulamento em revisão desde jan/2026). Albufeira em revisão desde fev/2026, sem texto novo — regra mantida
- [x] **1.5 I8 — verificador do número de registo AL** (Reg. UE 2024/1028) — `lib/rnal.ts`: normaliza (`n.º 12345 al` → `12345/AL`) e verifica a forma, com o cuidado de **não coser dois grupos de dígitos** (`12345/AL/2024` viraria `123452024/AL`, bem formado e falso). O cofre passa a ter estado `invalido`, que conta como crítico e vem antes de «em falta» na ordenação — quem não tem registo sabe-o, quem tem um errado julga estar em dia. `/api/compliance` grava a forma canónica e nunca recusa a gravação (trancava o anfitrião fora do resto do cofre). 15+3 testes. **Verifica a forma, não a existência**: não há serviço público que permita consultar o RNAL, e isso está escrito no módulo e dito ao anfitrião
- [x] **O RNAL faltava no site do anfitrião** — encontrado ao fazer 1.5: o DL 128/2014 obriga a que o número conste da publicidade do estabelecimento, e o site `/r/[slug]` nunca o mostrou em nenhuma das nove páginas. Agora aparece no rodapé (`adminGetRegistosAl`, só os que passam na verificação, PT/EN). Vai buscá-lo no rodapé em vez de o receber por prop para que a próxima página não nasça sem ele
- [x] **1.6 Mapa fiscal IRS Cat. B vs Cat. F + pacote para o contabilista** — **2026-09-25**. `/financeiro/fiscal` (atalho no cabeçalho do financeiro): por estabelecimento, modalidade RNAL, área de contenção e VPT (migração 047, aplicada); simula o imposto **incremental** de cada opção sobre o rendimento coletável do agregado, com os escalões de 2026, solidariedade e quociente conjugal. Regras e fontes em `lib/fiscal-irs.ts` (CIRS arts. 28.º n.º 14, 31.º n.º 1 a)/c)/h) e n.º 13, 41.º, 68.º, 68.º-A, 69.º, 72.º n.º 1 e); IAS 2026), todas lidas no Portal das Finanças; testes contra a coluna B da tabela do art. 68.º. Pacote CSV (`lib/pacote-contabilista.ts`): resumo por estabelecimento, reservas e despesas, com os critérios escritos no próprio ficheiro. **Fora, e dito na página**: Segurança Social, deduções à coleta, mais-valias da afetação do imóvel. Parâmetros só para 2026 — `parametrosDoAno` devolve `null` noutro ano em vez de inventar
