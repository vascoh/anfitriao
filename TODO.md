...
- [ ] **`RESEND_API_KEY`** — ✅ **2026-09-15: o DNS está todo de pé** (DKIM `resend._domainkey`, SPF `send.` com `include:amazonses.com`, MX `feedback-smtp.eu-west-1`, DMARC `p=none`), confirmado por consulta direta. Não há propagação por esperar nem registo por criar: falta **só** gerar a chave em resend.com/api-keys, colá-la em Production e voltar a deployar. Sem ela **não sai um único email** em produção (`NoopProvider` engole tudo; confirmado nos logs de arranque a 12/08). Afeta pedidos e confirmações de reserva, check-in, lembretes de pagamento, fim de trial, alertas de conformidade, relatório mensal e o motor de automações. ✅ **`EMAIL_FROM` já está definida** em produção desde 12/08 (confirmado por `vercel env ls production` a 07/09) — resta o remetente estar verificado no Resend, senão a chave entra e os emails ficam a ser recusados na mesma
