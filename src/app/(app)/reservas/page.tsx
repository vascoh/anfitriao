...
type Filter = 'todas' | 'ativas' | 'proximas' | 'passadas' | 'pendentes'
...
function filterBookings(bookings: Booking[], filter: Filter): Booking[] {
  ...
  switch (filter) {
    ...
    case 'passadas': return bookings.filter(b => b.estado === 'checkout' || b.estado === 'cancelada' || b.estado === 'no_show')
  ...
  return (
    <Link href={`/reservas/${b.id}`}
      ...
      <div className="h-9 w-9 rounded-full bg-primary/10 flex items-center justify-center shrink-0 mt-0.5">
        <span className="text-sm font-bold text-primary">{guest?.nome?.[0] ?? '?'}</span>
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          <span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full border ${STATUS_CLASS[b.estado]}`}>
            {STATUS_LABEL[b.estado]}
          </span>
          <span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full ${SOURCE_BG[b.origem]}`}>
        ...
        {saldo > 0 && (
          <p className="text-xs text-destructive mt-1">{fmtMoney(saldo)} por receber</p>
        )}
      </div>
    </Link>
  )
...
export default function ReservasPage() {
  ...
  const counts: Partial<Record<Filter, number>> = useMemo(() => ({
      ...
      <div className="flex-1">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 text-center py-16 px-4">
            ...
            {filter === 'todas' && (
              <Link href="/reservas/nova"
                className="mt-2 flex items-center gap-2 bg-primary text-primary-foreground rounded-lg px-4 py-2.5 text-sm font-medium">
                <Plus className="h-4 w-4" /> Nova reserva
              </Link>
            )}
          </div>
        ) : (
          <div className="rounded-none bg-card border-b border-border">
            {filtered.map(b => (
