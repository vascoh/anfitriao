...
interface ConfirmationSummary {
  booking: { check_in: string; check_out: string; num_hospedes: number; preco_total: number }
  property: { nome: string; imagem_url: string | null; cor: string } | null
  settings: { email: string | null; telefone: string | null; slug: string | null } | null
}
...
function ConfirmacaoInner() {
  ...
  const sessionId = searchParamsObj.get('session_id') ?? undefined
  const [data, setData] = useState<ConfirmationSummary | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      const res = await fetch(`/api/book-confirmation/${bookingId}`)
      if (res.ok) return res.json()

      // Pagamento com cartão: a reserva só existe depois de o pagamento ser
      // confirmado — o webhook pode ainda não ter corrido. Tenta preencher
      // de forma síncrona (idempotente) antes de desistir.
      if (sessionId) {
        const fulfillRes = await fetch(
          `/api/book-checkout-fulfill?session_id=${encodeURIComponent(sessionId)}&propriedade_id=${encodeURIComponent(params.propertyId)}`,
        )
        if (fulfillRes.ok) {
          const retry = await fetch(`/api/book-confirmation/${bookingId}`)
  ...
  }, [bookingId, sessionId, params.propertyId])
  ...
  const waLink = settings?.telefone
    ? `https://wa.me/${settings.telefone.replace(/\D/g, '')}?text=${encodeURIComponent(
        (sessionId ? `Olá! Acabei de pagar e reservar em ${prop?.nome ?? 'alojamento'}` : `Olá! Acabei de submeter um pedido de reserva em ${prop?.nome ?? 'alojamento'}`) +
  ...
  return (
    <div className="min-h-dvh bg-background flex flex-col">
      ...
      <header className="px-4 py-3 border-b border-border flex items-center gap-3">
        <Link href={backHref} className="p-1 -ml-1 rounded-lg text-muted-foreground hover:text-foreground">
        ...
        <div className="w-full max-w-md flex flex-col items-center gap-6 text-center">
          ...
          <div className="flex flex-col gap-2">
            <h1 className="text-2xl font-bold">
              {nome ? `Obrigado, ${nome.split(' ')[0]}!` : sessionId ? 'Reserva confirmada!' : 'Pedido enviado!'}
            </h1>
            <p className="text-muted-foreground text-sm leading-relaxed max-w-sm">
              {sessionId
                ? 'O pagamento foi confirmado e a tua reserva está garantida. Enviámos os detalhes para o teu email.'
                : 'O seu pedido de reserva foi recebido com sucesso. Entraremos em contacto brevemente para confirmar os detalhes e combinar o pagamento.'}
            </p>
            {paymentFailed && (
              <p className="text-xs text-destructive">
                Não conseguimos confirmar o pagamento automaticamente. Se o valor foi cobrado, contacta o anfitrião — a reserva será confirmada manualmente.
              </p>
            )}
          </div>
            ...
            <div className="w-full rounded-2xl border border-border bg-card overflow-hidden">
              ...
              <div className="p-4 flex flex-col gap-3 text-left">
                ...
                <p className="text-xs text-muted-foreground">
                  {sessionId ? 'Pagamento confirmado com cartão · Sem taxas de serviço' : 'Sem taxas de serviço · Pagamento acordado com o anfitrião'}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="w-full flex flex-col gap-3">
            {waLink && (
              <a href={waLink} target="_blank" rel="noopener noreferrer"
                ...
                {sessionId ? 'Contactar via WhatsApp' : 'Confirmar via WhatsApp'}
              </a>
            )}
