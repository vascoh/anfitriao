...
export default async function TermosPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div lang={htmlLang(lang)} className={`min-h-dvh bg-background flex flex-col ${theme.className}`} style={theme.style}>
      ...
      <LegalPage title={t(lang, 'legal_terms_title')} lang={lang}>
        <TermosTexto lang={lang} nome={settings.nome} minNoites={settings.min_noites} contacto={settings.email || settings.telefone || null} />
      </LegalPage>
